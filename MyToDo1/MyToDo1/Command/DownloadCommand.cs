...
namespace MyToDo1.Command
{
    ...
    {
        private nint _directoryItem = nint.Zero;

        public DownloadCommand(ref CameraModel model, ref nint inRef) : base(ref model)
        {
        ...
        ~DownloadCommand()
        {
            if (_directoryItem != nint.Zero)
            {
                EDSDKLib.EDSDK.EdsRelease(_directoryItem);
                _directoryItem = nint.Zero;
            }
        }
        ...
        private uint ProgressFunc(uint inPercent, nint inContext, ref bool outCancel)
        {
            _event = new CameraEvent(CameraEvent.Type.PROGRESS, (nint)inPercent);
