    ...
    class GetPropertyCommand : Command
    {
        ...
        private uint GetProperty(uint propertyID)
        {
            ...
            if (propertyID == EDSDKLib.EDSDK.PropID_Unknown)
            {
                ...
                if (err == EDSDKLib.EDSDK.EDS_ERR_OK) err = GetProperty(EDSDKLib.EDSDK.PropID_PictureStyle);
                if (err == EDSDKLib.EDSDK.EDS_ERR_OK) err = GetProperty(EDSDKLib.EDSDK.PropID_MovieHFRSetting);
            ...
            if (err == EDSDKLib.EDSDK.EDS_ERR_OK)
            {
                CameraEvent e = new CameraEvent(CameraEvent.Type.PROPERTY_CHANGED, (nint)propertyID);
                _model.NotifyObservers(e);
            }
            return err;
        }
