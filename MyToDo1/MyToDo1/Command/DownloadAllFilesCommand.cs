...
namespace MyToDo1.Command
{
    ...
    {
        public DownloadAllFilesCommand(ref CameraModel model, ref nint volume) : base(ref model) { _volume = volume; }
        ...
        private static List<nint> _imageItems = new List<nint>();
        ...
        public override bool Execute()
        {
            ...
            if (_currentFileNum == 0)
            {
                ...
                // Show progress.
                _event = new CameraEvent(CameraEvent.Type.DOWNLOAD_START, _fileCount);
        ...
        private uint DownloadImageByDirectory(nint camera)
        {
            nint fileitem;
            ...
            for (index = 0; index < _imageItems.Count; ++index)
            {
                ...
                {
                    _event = new CameraEvent(CameraEvent.Type.DOWNLOAD_COMPLETE, index);
                    _model.NotifyObservers(_event);
                    _currentFileNum = 0;
                    _imageItems.Clear();
                }
                ...
                {
                    _event = new CameraEvent(CameraEvent.Type.PROGRESS_REPORT, _currentFileNum);
                    _model.NotifyObservers(_event);
                }
            }
