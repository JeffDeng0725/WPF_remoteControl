...
namespace MyToDo1.Command
{
    ...
    {
        public DeleteAllFilesCommand(ref CameraModel model, ref nint volume) : base(ref model) { _volume = volume; }

        private static FileCounterCommand.FileNumber _fileNumber;
        private static int _currentFileNum = 0;
        private static List<nint> _imageItems;
        ...
        public override bool Execute()
        {
            ...
            if (_currentFileNum == 0)
            {
                ...
                _event = new CameraEvent(CameraEvent.Type.DELETE_START, fileCount);
                ...
                if (fileCount == 0)
                {
                    _event = new CameraEvent(CameraEvent.Type.DELETE_COMPLETE, fileCount);
                    _model.NotifyObservers(_event);
                }
        ...
        private uint DeleteFileByDirectory(nint camera)
        {
            int index = 0;
            ...
            for (index = 0; index < _imageItems.Count; ++index)
            {
                ...
                {
                    _event = new CameraEvent(CameraEvent.Type.DELETE_COMPLETE, index);
                    _model.NotifyObservers(_event);
                    _currentFileNum = 0;
                    // Stop always.
                    _imageItems.Clear();
                ...
                {
                    _event = new CameraEvent(CameraEvent.Type.PROGRESS_REPORT, index);
                    _model.NotifyObservers(_event);
                }
