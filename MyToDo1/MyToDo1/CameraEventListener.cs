...
namespace MyToDo1
{
    ...
    {
        public static uint HandleObjectEvent(uint inEvent, nint inRef, nint inContext)
        {
        ...
        public static uint HandlePropertyEvent(uint inEvent, uint inPropertyID, uint inParam, nint inContext)
        {
            GCHandle handle = GCHandle.FromIntPtr(inContext);
            CameraController controller = (CameraController)handle.Target;
            ...
            switch (inEvent)
            {
                case EDSDKLib.EDSDK.PropertyEvent_PropertyChanged:
                    FireEvent(ref controller, ActionEvent.Command.GET_PROPERTY, (nint)inPropertyID);
                    break;

                case EDSDKLib.EDSDK.PropertyEvent_PropertyDescChanged:
                    FireEvent(ref controller, ActionEvent.Command.GET_PROPERTYDESC, (nint)inPropertyID);
                    break;
            }
            return EDSDKLib.EDSDK.EDS_ERR_OK;
        }
        ...
        public static uint HandleStateEvent(uint inEvent, uint inParameter, nint inContext)
        {
            GCHandle handle = GCHandle.FromIntPtr(inContext);
            CameraController controller = (CameraController)handle.Target;
            ...
            switch (inEvent)
            {
                case EDSDKLib.EDSDK.StateEvent_Shutdown:
                    FireEvent(ref controller, ActionEvent.Command.SHUT_DOWN, nint.Zero);
                    break;
            }
            return EDSDKLib.EDSDK.EDS_ERR_OK;
        }

        private static void FireEvent(ref CameraController controller, ActionEvent.Command command, nint arg)
        {
